<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "INSERT INTO Clients (Client_Name,Address,Phone_Number,Case_Number,Case_Manager,Hours,Gaurdian,Birthday,Notes) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($name,$address,$phone_number,$case_number,$case_manager,$hours,$gaurdian,$birthday,$notes));
			Database::disconnect();
